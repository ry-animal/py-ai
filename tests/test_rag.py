...
def test_rag_reload_and_ask() -> None:
    ...
    with patch("app.rag_service.RAGService.ingest", return_value=2) as _ingest:
        res = client.post(
            ...
            json=[["1", "hello world"], ["2", "another doc"]],
        )
        assert res.status_code == 200
        assert res.json()["ingested"] == 2

    with (
        patch("app.rag_service.RAGService.retrieve", return_value=["hello world"]) as _ret,
        patch("app.ai_service.AIService.generate_answer", new=AsyncMock(return_value="hi")),
    ):
...
def test_rag_ask_streams() -> None:
    ...
    with patch("app.rag_service.RAGService.retrieve", return_value=["ctx"]) as _ret:
        ...
        with patch(
            "app.ai_service.AIService.generate_answer",
...
def test_rag_reload_background_enqueues_task() -> None:
    client = TestClient(app)

    with patch("app.tasks.reload_rag_task.delay", return_value=MagicMock(id="task-1")):
        res = client.post(
            "/rag/reload",
            json=[["doc", "text"]],
        )
